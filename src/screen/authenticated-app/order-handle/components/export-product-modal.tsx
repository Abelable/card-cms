...
import { useExportProduceProduct } from "service/produce";
...
export const ExportProductModal = ({
  ...
  const exportProduceProduct = useExportProduceProduct();

  const exportFilter = () => {
    form.validateFields().then(async () => {
      const { type, agentId, encodings, password } = form.getFieldsValue();
      if (type === 1) {
        exportProduceProduct({ agentId, password });
      } else {
        exportProduceProduct({ encodings, password });
      }
      closeModal();
    });
  };
  ...
  const exportAll = () => {
    form.validateFields().then(async () => {
      exportProduceProduct({ password: form.getFieldsValue().password });
      closeModal();
    });
  };
